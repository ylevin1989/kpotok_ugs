...
def seed_job_fixture(title: str):
    ...
    db = SessionLocal()
    org = Organization(name='Uno Packet119', slug=f"uno-packet119-{title.replace(' ', '-').lower()}", status=OrganizationStatus.ACTIVE)
    db.add(org)
    db.flush()
    brand = Brand(organization_id=org.id, name='Rocket Tea 119', slug=f"rocket-tea-p119-{title.replace(' ', '-').lower()}")
    db.add(brand)
    db.flush()
    brief = Brief(organization_id=org.id, brand_id=brand.id, title='Launch brief 119', content='Need a campaign launch plan 119.')
    db.add(brief)
    db.flush()
...
def test_execution_trace_exposes_attempt_tag_count_and_compact_counts(client):
    owner, job_id = seed_job_fixture('Packet 119 compact counts and worker scope')
    ...
    assert trace['attempt_summary']['unique_transition_tag_count'] == 2
    assert trace['trace_compact_summary']['progress_history_sample_count'] == 2
    assert trace['trace_compact_summary']['unique_stage_count'] == 2
    assert trace['trace_compact_summary']['worker_count'] == 1


def test_terminal_completed_event_carries_stage_labels_and_progress_floor(client):
    owner, job_id = seed_job_fixture('Packet 119 terminal stage labels and floor')
    ...
    assert completed_event['timeline_digest']['first_stage_label'] == 'Drafting'
    assert completed_event['timeline_digest']['latest_progress_percent'] == 80
    assert completed_event['progress_digest']['min_progress_percent'] == 25
    assert completed_event['progress_digest']['progress_remaining_percent'] == 20
    assert completed_event['progress_window']['average_progress_percent'] == 52.5


def test_reclaim_failed_event_carries_reclaim_and_attempt_totals(client):
    owner, job_id = seed_job_fixture('Packet 119 reclaim attempt totals')
    ...
    assert failed_event['failure_digest']['reclaim_count'] == 1
    assert failed_event['failure_digest']['worker_count'] == 2
    assert failed_event['worker_recap']['attempt_count'] == 2
