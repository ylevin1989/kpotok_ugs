...
def seed_job_fixture(title: str):
    ...
    db = SessionLocal()
    org = Organization(name='Uno Packet53', slug=f"uno-packet53-{title.replace(' ', '-').lower()}", status=OrganizationStatus.ACTIVE)
    db.add(org)
    db.flush()
    brand = Brand(organization_id=org.id, name='Rocket Tea 53', slug=f"rocket-tea-p53-{title.replace(' ', '-').lower()}")
    db.add(brand)
    db.flush()
    brief = Brief(organization_id=org.id, brand_id=brand.id, title='Launch brief 53', content='Need a campaign launch plan 53.')
    db.add(brief)
    db.flush()
...
def test_execution_trace_exposes_attempt_duration_and_stage_transition_counts(client):
    owner, job_id = seed_job_fixture('Packet 53 attempt duration and transitions')
    ...
    assert trace['stage_transition_counts'] == {
        'claimed': 1,
        'generate-copy': 2,
        'persist-artifact': 1,
    }
    assert trace['attempt_summary']['attempt_number'] == 1
    assert trace['attempt_summary']['progress_event_count'] == 3
    assert trace['attempt_summary']['last_stage_name'] == 'persist-artifact'
    assert trace['attempt_summary']['last_progress_sequence'] == 3
    assert trace['attempt_summary']['attempt_duration_seconds'] >= 0.02


def test_terminal_events_carry_trace_summary(client):
    owner, job_id = seed_job_fixture('Packet 53 terminal trace summary')
    ...
    assert failed_event['trace_summary'] == {
        'final_status': 'failed',
...
def test_reclaim_resets_attempt_duration_for_new_attempt(client):
    owner, job_id = seed_job_fixture('Packet 53 reclaim resets attempt duration')
    ...
    assert trace['attempt_summary']['progress_event_count'] == 1
    assert trace['attempt_summary']['last_progress_percent'] == 15
    assert trace['attempt_summary']['attempt_duration_seconds'] >= 0.02
    assert trace['stage_transition_counts'] == {
        'claimed': 1,
        'generate-copy': 1,
