...
def seed_jobs_fixture():
    owner = seed_user('owner-p30@example.com')
    db = SessionLocal()
    org = Organization(name='Uno Packet30', slug='uno-packet30', status=OrganizationStatus.ACTIVE)
    db.add(org)
    db.flush()
    brand = Brand(organization_id=org.id, name='Rocket Tea 30', slug='rocket-tea-p30')
    db.add(brand)
    db.flush()
    brief = Brief(organization_id=org.id, brand_id=brand.id, title='Launch brief 30', content='Need a campaign launch plan 30.')
    db.add(brief)
    db.flush()
    db.add(OrganizationMembership(organization_id=org.id, user_id=owner.id, role=MembershipRole.CLIENT_OWNER))
    from app.db.models.job import Job
    job = Job(organization_id=org.id, brand_id=brand.id, brief_id=brief.id, title='Generate artifacted launch copy', status='queued')
    db.add(job)
    db.commit()
    db.refresh(job)
    org_id = str(org.id)
...
def test_complete_persists_artifact_reference_and_public_readback(client):
    ...
    assert completed['output_text'] == 'artifacted-output'
