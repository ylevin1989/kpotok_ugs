...
def seed_job_fixture(title: str):
    ...
    db = SessionLocal()
    org = Organization(name='Uno Packet56', slug=f"uno-packet56-{title.replace(' ', '-').lower()}", status=OrganizationStatus.ACTIVE)
    db.add(org)
    db.flush()
    brand = Brand(organization_id=org.id, name='Rocket Tea 56', slug=f"rocket-tea-p56-{title.replace(' ', '-').lower()}")
    db.add(brand)
    db.flush()
    brief = Brief(organization_id=org.id, brand_id=brand.id, title='Launch brief 56', content='Need a campaign launch plan 56.')
    db.add(brief)
    db.flush()
...
def test_execution_trace_exposes_progress_velocity_and_dominant_stage(client):
    owner, job_id = seed_job_fixture('Packet 56 velocity and dominant stage')
    ...
    assert trace['dominant_stage_name'] == 'generate-copy'
    assert trace['attempt_summary']['progress_velocity_percent_per_second'] > 0
    assert trace['attempt_summary']['progress_velocity_percent_per_second'] == trace['attempt_summary']['progress_velocity_percent_per_second']


def test_terminal_events_carry_progress_digest(client):
    owner, job_id = seed_job_fixture('Packet 56 terminal progress digest')
    ...
    assert failed_event['progress_digest']['last_progress_percent'] == 80
    assert failed_event['progress_digest']['last_progress_sequence'] == 1
    assert failed_event['progress_digest']['progress_event_count'] == 1
    assert failed_event['progress_digest']['dominant_stage_name'] == 'persist-artifact'
    assert failed_event['progress_digest']['progress_velocity_percent_per_second'] > 0


def test_reclaim_resets_dominant_stage_and_velocity(client):
    owner, job_id = seed_job_fixture('Packet 56 reclaim reset digest')
