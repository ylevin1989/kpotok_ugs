...
def seed_job_fixture(title: str):
    ...
    db = SessionLocal()
    org = Organization(name='Uno Packet109', slug=f"uno-packet109-{title.replace(' ', '-').lower()}", status=OrganizationStatus.ACTIVE)
    db.add(org)
    db.flush()
    brand = Brand(organization_id=org.id, name='Rocket Tea 109', slug=f"rocket-tea-p109-{title.replace(' ', '-').lower()}")
    db.add(brand)
    db.flush()
    brief = Brief(organization_id=org.id, brand_id=brand.id, title='Launch brief 109', content='Need a campaign launch plan 109.')
    db.add(brief)
    db.flush()
...
def test_execution_trace_exposes_min_progress_stage_counts_and_compact_averages(client):
    owner, job_id = seed_job_fixture('Packet 109 min progress and compact averages')
    ...
    assert trace['attempt_summary']['min_progress_percent'] == 25
    assert trace['attempt_summary']['unique_stage_count'] == 2
    assert trace['trace_compact_summary']['average_progress_percent'] == 53.333333333333336
    assert trace['trace_compact_summary']['unique_transition_tag_count'] == 2


def test_terminal_completed_event_carries_first_sequence_latest_label_and_progress_averages(client):
    owner, job_id = seed_job_fixture('Packet 109 terminal averages and labels')
    ...
    assert completed_event['timeline_digest']['first_progress_sequence'] == 1
    assert completed_event['timeline_digest']['latest_stage_label'] == 'Upload'
    assert completed_event['progress_digest']['average_progress_percent'] == 52.5
    assert completed_event['progress_window']['min_progress_percent'] == 25


def test_reclaim_failed_event_carries_had_reclaim_and_reclaim_count(client):
    owner, job_id = seed_job_fixture('Packet 109 reclaim recap count')
    ...
    assert failed_event['failure_digest']['had_reclaim'] is True
    assert failed_event['worker_recap']['reclaim_count'] == 1
